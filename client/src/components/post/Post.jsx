...
const Post = () => {
  ...
  const loadPublisher = (post) => {
    axios
      .get("users/profile/" + post.userId)
      .then((res) =>
        setPublisherName(res.data.user.firstName + " " + res.data.user.lastName)
      )
  ...
  return (
    ...
    >
      <Stack direction="column" spacing={1}>
        <Badge
          color={isDeleted ? "error" : "secondary"}
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Stack
          ...
          sx={{ marginTop: "7%" }}
        >
          <Button variant="contained" type="submit" disabled={isDeleted}>
